...
class WaveGuideTrainer:
    def __init__(self, dtype, device, memory_type,samplers, optimzer, scheduler, n_upper=None, n_lower=None, dx=None, num_samples=None):
      ...
      sorted_RIs = np.sort(-device.RIs)

      if n_lower is None:
        self.n_lower = sorted_RIs[1]
      else:
        self.n_lower = n_lower
      if n_upper is None:
        self.n_upper = sorted_RIs[0]
      else:
    ...
    def continuity_loss(self, x, u, rayleigh_values, num_modes):
        ...
        discontinuities_tensor = self.device.get_discontinuities(format='torch').to(self.memory_type).reshape(-1, 1)
        ...
        refractive_indices_squared = torch.from_numpy(self.device.RIs).to(self.memory_type).reshape(-1, 1) ** 2
    ...
    def interior_loss(self, x_rayleigh, x_PDE, u, num_modes, sample_new=False):
        ...
        for i in range(num_modes):
            ...
            if not sample_new :
                ...
                new_samples = WaveGuideTrainer.adaptive_sampling(x_PDE, residuals, sample_new * len(x_rayleigh), self.device)
