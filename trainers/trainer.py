...
class Trainer:
    ...
    def continuity_loss(self, x, u, rayleigh_values, num_modes):
        ...
        refractive_indices_squared_in = torch.square(self.guide.refractive_index(discontinuities_tensor.detach()+biases, format='torch'))
        refractive_indices_squared_out = torch.square(self.guide.refractive_index(discontinuities_tensor.detach()-biases, format='torch'))
        ...
        in_vals = self.guide.underlying_model(interface_input_in)
        out_vals = self.guide.underlying_model(interface_input_out)
    ...
    def interior_loss(self, x_rayleigh, x_PDE, u, num_modes, sample_new=False, u_ref=None):
        ...
        for i in range(num_modes): # num_modes = 1 if we are working with sequential mode solving (relying on u_ref)
            ...
            if sample_new :
                new_samples = Trainer.adaptive_sampling(x_PDE, residuals, sample_new * len(x_rayleigh), self.guide)

            energy = torch.sum(u[:, i] ** 2) * self.guide.total_length / len(u[:])
            losses_2.append(1/torch.square(energy))

            if rayleigh < self.n_lower**2:
                losses_3.append(torch.square((rayleigh - self.n_upper**2)/(self.n_upper**2 - self.n_lower**2))/(i+1))
            else:
                losses_3.append(1/(rayleigh/a - b)/(i+1))
    ...
    def construct_training_data(self):
        ...
        if self.guide.augmented_input:
            ...
            x_PDE = torch.cat((x_PDE, 1/n_squared), dim=1).to(self.device)
        if len(self.samplers)==2:
          ...
          if self.guide.augmented_input:
              ...
              x_rayleigh = torch.cat((x_rayleigh, 1/n_squared_rayleigh), dim=1).to(self.device)
        else:
          x_rayleigh = x_PDE
    ...
    def train(self, epochs, num_modes, weights=[1, 1, 1, 1, 1, 1], verbose=0, calc_error=False, plot_solution=False, sample_new=False, u_ref=[]):
        ...
        if self.guide.augmented_input:
            ...
            bd = torch.cat((bd, 1/n_squared_bd), dim=1).to(self.device)
        ...
        for j in loop:
            ...
            if self.guide.augmented_input:
                noise[:, 1] = 0
            u_pred = self.guide.underlying_model(x_PDE + noise)
        ...
        return loss_history, rayleigh_history, mode_errors_list, u_pred.to(self.device).detach().squeeze()